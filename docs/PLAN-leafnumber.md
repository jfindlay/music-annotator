...
| L5 | done    | 70a2a41 | — (◆ capstone) | NOTES.md: four leaf-numbering invariants named (leaf=`CWP_MOVT_NUM` C-L0; intermediate=`CWP_INTER_INDEX_{i}` C-L1; one-depth-per-group DESIGNED+DEFERRED L2; `repathed` journal obligation C-L4). Bug entry annotated RESOLVED (L0/L3) for Findings 1+3, DEFERRED (L2) for Finding 4; retroactive-maintenance note corrected (`repath` stands alone on `_pipeline`/`_pipeline_io` primitives; audit/regroup is a not-yet-built sibling). README.md: `repath` subcommand documented (warning + offline/provenance prose + examples); stale ordering-key layout bullet corrected. Files: `docs/NOTES.md`, `README.md`. (Prose; no gate.) |
